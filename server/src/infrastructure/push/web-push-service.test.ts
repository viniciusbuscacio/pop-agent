 ...
 * web.push.apple.com on 01/08/2026: `mailto:popy@localhost` -> 403,
 * `https://github.com/viniciusbuscacio/popy` -> 201.
 */

const DEFAULT = 'https://github.com/viniciusbuscacio/popy';
...
describe('vapidSubject', () => {
  ...
  it('keeps an operator address that Apple will accept', () => {
    ...
    expect(vapidSubject('  mailto:ops@popy.example.org  ')).toBe('mailto:ops@popy.example.org');
    expect(vapidSubject('https://popy.example.com/contact')).toBe('https://popy.example.com/contact');
  });
  ...
  it('drops what would be rejected upstream rather than honouring it', () => {
    ...
    expect(vapidSubject('mailto:popy@localhost')).toBe(DEFAULT);
    ...
    expect(vapidSubject('popy@example.com')).toBe(DEFAULT); // no scheme

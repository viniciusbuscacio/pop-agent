...
export const POP_LOCAL_ACCESS_SKILL: DefaultSkill = {
  ...
  body: [
    ...
    '',
    'On Windows and macOS, Pop Agent Setup offers Pop Agent Desktop and Pop Agent CLI, both selected by default. Desktop includes a WebView2 (Windows) or WKWebView (macOS) window and computer-access tray; CLI adds the pop terminal command. Desktop-only installs use a private runtime. The web window uses its own ordinary sign-in and exposes no native tool bindings. Existing components are preserved during upgrades. The macOS DMG contains Pop Agent Setup.app with Continue/Install/Finish. It supports fresh Desktop/CLI installation and legacy PLA migration. Desktop uses a nested helper bundle; CLI alone adds an owned zsh/bash PATH block. The existing browser PWA remains independent.',
    '',
    '## User update flow',
    '',
    'The menu has four top-level entries: Open Pop Agent, Computer access, Settings, and Quit Pop Agent Desktop. Computer access contains the permission switch and native Sign in again where supported. Settings contains Start at Login, updates, and Troubleshooting (Reconnect, Open logs, server address). Above these actions, Server connected with a green dot indicates a live server transport independently of file permission; Server disconnected uses a neutral dot. Settings · Update available keeps available updates visible at the top level. The tray checks its configured Pop Server at startup, every six hours, and through Check for updates. A newer installer produces Update available…. Clicking downloads and verifies the matching installer from that same server, then opens it. No installer is downloaded automatically, and replacement requires the native installer confirmation. macOS uses a DMG containing Pop Agent Setup.app; Windows uses a setup.exe. Current component installers support fresh installation and updates. Older Mac DMGs may be update-only: inspect the installed/released version before promising the new wizard. Installation remains explicitly confirmed.',
    '',
    ...
    '',
    'Windows executables cross-compile on Linux. Complete releases require macOS binaries and DMGs built on a Mac with Xcode, Go, Python and hdiutil from the exact clean release commit, then transferred into release-builder/cache/macos-tray/<commit>. The explicit owner-authorized `build-windows` mode may omit macOS PLA from one immutable version so Windows can be validated first; a missing Mac directory alone never selects this mode, macOS discovery returns 404, and restoring macOS requires a new version. An existing DMG filename is not proof that the current server publishes it.',
    '',

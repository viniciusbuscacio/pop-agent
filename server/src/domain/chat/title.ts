...
export function fallbackTitle(text: string, taken: readonly string[] = []): string {
  ...
  if (chosen.length === 0) return unique('New chat', taken);

  const title = chosen.map(titleCase).join(' ').slice(0, MAX_LENGTH).trim();
  return unique(title, taken);
}
...
/** "Groceries", then "Groceries 2", "Groceries 3"... */
function unique(title: string, taken: readonly string[]): string {

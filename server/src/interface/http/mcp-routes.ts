...
function toDto(s:McpServer&{capabilities:McpCapability[]}):McpServerDTO{return {id:s.id,name:s.name,description:s.description,transport:s.transport,endpoint:s.endpoint,command:s.command,args:s.args,authKind:s.authKind,authHeader:s.authHeader,enabled:s.enabled,timeoutMs:s.timeoutMs,status:s.status,lastError:s.lastError,...(s.lastConnectedAt===undefined?{}:{lastConnectedAt:s.lastConnectedAt}),...(s.protocolEra===undefined?{}:{protocolEra:s.protocolEra}),...(s.protocolVersion===undefined?{}:{protocolVersion:s.protocolVersion}),cwd:s.cwd,capabilities:s.capabilities.map(capabilityDto)};}

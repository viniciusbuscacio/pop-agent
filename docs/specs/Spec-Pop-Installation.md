...
uploads succeed. A failed upload leaves an unpublished draft. Repeating publish with the same
verified batch resumes only when the tag resolves to that commit. Existing
assets must match size and SHA-256; only missing assets are uploaded. Empty
failed-upload starters may be removed from a matching draft. Conflicting
assets, unexpected files and published releases are never overwritten.
...
The normal Desktop installation path does not require copying a Terminal command.
For macOS blocking the ad-hoc-signed setup, show an optional copyable Terminal
command below the DMG downloads. Require the Pop Agent Setup volume to remain
mounted; copy its app into a unique temporary directory, verify the copy with
`codesign --verify --deep --strict`, then remove quarantine only from that copy
and open it. Chain the steps with `&&` so a failure stops the sequence.
...
Client compatibility fingerprints include packaging scripts and helpers, Node
runtime pins and TypeScript build configuration as well as client source and
dependencies. When this inventory changes, regenerate the pinned snapshot from
a verified published release; never substitute the working tree hash by hand.

## Clean 0.3.0 distribution baseline

Version 0.3.0 starts a new Git history. Its server/web version advances while
verified client bytes retain their independent original versions. The initial
release contains the verified client snapshot archive and native assets at new
0.3.0 download URLs, so older tags and releases are not required. The snapshot
retains the original build commit/tree identifiers only as provenance, without
retaining the old Git objects or history. After publication, the usual pin

...
Pop Agent is single-owner. Server settings expose one Access key, Copy key and Generate new key. There is no token list, pagination, name, expiry or permission form. Agent instructions include the actual current key and copy as a ready-to-paste block. Startup creates a key if missing before exposing the server, matching go-notepad defaults. Enabling Server ensures the key is saved before the switch becomes enabled; a write failure leaves a stopped server stopped. The owner key read repairs a missing key as well. Existing keys are reused across screen reopening, stop/start and process restart; only Generate new key deliberately rotates them. Copy is disabled while the key loads. Replacing the key immediately updates the instructions. A failed/lost rotation response triggers one read-back, never automatic rotation retries.

Owner-only GET `/v1/rest-api/key` returns `{secret: string}`; POST generates/replaces it and returns the new secret. Both responses use `Cache-Control: no-store`. A key is a random 256-bit opaque `popi_` bearer. Its SHA-256 digest is used for authentication; its recoverable value is stored encrypted through SecretsRepo, never in plain settings, logs or browser persistence. It has no automatic expiry (the existing numeric metadata uses the maximum JavaScript date). Only owner sessions may read or replace the key, including while Server is disabled.

Rotation atomically revokes every existing token, inserts the new authentication digest and saves its encrypted value in the same SQLite transaction. Persistence failure retains the previous key. The new key grants all five REST scopes internally: activity, conversation read/write, cancellation and UI control. No per-key permission selector is shown. Existing scoped tokens are invalidated when startup initializes the single key; they are not upgraded. The old POST `/v1/rest-api/tokens` returns 410 `single_api_key`; legacy metadata GET and DELETE remain for migration, but are absent from the product UI. DELETE cannot revoke the current singleton key (409 `replace_api_key`); replace it through the key endpoint instead. Once a singleton exists, legacy scoped issuance is also rejected internally.

...
Tokens only enter `/v1/integration/*`. Every operation enforces its scope: `activity:read`, `conversations:read`, `conversations:write`, `runs:cancel`, or `ui:control`. Write does not imply read. Activity alone exposes IDs, timestamps, execution state, tool name/status and coarse phase; no titles, prompts, model output, tool arguments or tool output. Read grants installation-wide personal conversation content. Conversation/activity scopes do not grant account administration or local machine selection. The separate opt-in `ui:control` scope operates an authenticated owner UI while REST API Server is enabled and therefore can perform owner actions through that tab.
...
`GET /v1/integration/ax` discovers the contract; `/v1/ax` is its owner-session equivalent. UI-scoped integration tokens use `/v1/integration/ui/sessions`, `/state?sessionId=...`, `/screenshot?sessionId=...`, and POST `/press`, `/dblclick`, `/input`, `/key`. Owner sessions use `/v1/ui/*`. UI access checks the module switch, token scope and rate limit, rechecks authorization before command delivery and before returning results. The UI scope explicitly grants owner actions and screen contents; it is never selected by default. Native text state excludes password values and elements marked `data-ui-private`. Screenshots are raw user-authorized captures and may contain visible secrets.
...
The Server editor follows the go-notepad layout with Start/Stop, persisted enabled-state status, address, connection port, HTTPS and access-control cards, Agent instructions and API tokens. No separate UI-access or screenshot-sharing panel is shown. Port/HTTPS describe the current app origin; this screen does not change the shared listener, TLS or tailnet policy. No independent API autostart switch is shown: the existing enabled state persists across restarts. Copy instructions uses a placeholder until a token is created; while its one-time secret is visible, it includes that token and only its granted operations. Detailed reference and token pagination remain intact.

...
func (i *ICoreWebView2) AddNavigationStarting(eventHandler *ICoreWebView2NavigationStartingEventHandler, token *_EventRegistrationToken) error {
	_, _, err := i.vtbl.AddNavigationStarting.Call(uintptr(unsafe.Pointer(i)), uintptr(unsafe.Pointer(eventHandler)), uintptr(unsafe.Pointer(token)))
	if err != windows.ERROR_SUCCESS {
		return err
	}
	return nil
...
func (i *ICoreWebView2) AddNewWindowRequested(eventHandler *ICoreWebView2NewWindowRequestedEventHandler, token *_EventRegistrationToken) error {
	_, _, err := i.vtbl.AddNewWindowRequested.Call(uintptr(unsafe.Pointer(i)), uintptr(unsafe.Pointer(eventHandler)), uintptr(unsafe.Pointer(token)))
	if err != windows.ERROR_SUCCESS {
		return err
	}
	return nil

...
func (t *winTray) iconToBitmap(hIcon windows.Handle) (windows.Handle, error) {
	...
	info := bitmapInfo{Header: bitmapInfoHeader{Width: int32(cx), Height: -int32(cy), Planes: 1, BitCount: 32, Compression: BI_RGB}}
	info.Header.Size = uint32(unsafe.Sizeof(info.Header))
	var bits unsafe.Pointer
	hMemBmp, _, err := pCreateDIBSection.Call(hDC, uintptr(unsafe.Pointer(&info)), DIB_RGB_COLORS, uintptr(unsafe.Pointer(&bits)), 0, 0)
	if hMemBmp == 0 {
		return 0, err
	}
	hOriginalBmp, _, _ := pSelectObject.Call(hMemDC, hMemBmp)
	defer pSelectObject.Call(hMemDC, hOriginalBmp)
	res, _, err := pDrawIconEx.Call(hMemDC, 0, 0, uintptr(hIcon), cx, cy, 0, 0, DI_NORMAL)
	if res == 0 {
		return 0, err
	}

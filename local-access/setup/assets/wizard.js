...
(() => {
  ...
  function show(next) {
    ...
    $('back').hidden = !['license', 'connection', 'destination'].includes(step);
    $('next').hidden = step === 'installing';
    text('next', { welcome:'Next', license:'I agree', connection:'Next', destination:'Install', done:'Finish', uninstall:'Uninstall' }[step] || 'Next');
  ...
  $('back').onclick = () => { error(''); show({ license:'welcome', connection:'license', destination:'connection' }[step]); };
  $('server').addEventListener('input', () => { $('password').placeholder = ''; });
  $('next').onclick = async () => {
    if (busy) return;
    error('');
    if (step === 'welcome') return show('license');
    if (step === 'license') return show('connection');
    if (step === 'connection') {
    ...
    if (step === 'destination') {
      ...
      try {
        const result = await api().Install($('server').value.trim(), $('password').value);
        $('password').value = '';
  ...
  async function start() {
    try {
      ...
      text('welcome-title', state.installed ? 'Update Pop Local Access' : 'Welcome to Pop Local Access');

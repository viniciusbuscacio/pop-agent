...
export function UiAccessPanel({ enabled = true, showExamples = true }: { enabled?: boolean; showExamples?: boolean }) {
  ...
  return <Card className="space-y-3">
    ...
    {!state.connected ? <><TextField id="ui-tab-name" label="Tab name" maxLength={80} value={name} onChange={e => setName(e.target.value)} /><Button type="button" data-testid="ui-connect" disabled={busy || !enabled || !name.trim()} onClick={() => void run(() => uiControl.start(name.trim()))}>Connect this tab</Button></> : <>
    ...
    {showExamples ? <details className="space-y-2"><summary className="cursor-pointer text-sm">API examples</summary>

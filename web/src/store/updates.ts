 ...
 * out. Device-scoped like the theme (popy.spec §14, §15): it never travels to
 * the server, so each device keeps its own schedule. Ten minutes by default
 * while Popy moves fast; the shipped default drops to daily (popy.spec §15).
 *
...
const STORAGE_KEY = 'popy.updateCheckMinutes';

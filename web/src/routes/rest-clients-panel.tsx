...
function ClientEditor({ client, onCancel, onSaved }: {
    ...
    <h4 className="font-medium">Allowed operations</h4>{operations.map((op, index) => <div key={index} className="space-y-2 border-t border-[var(--border)] pt-3">
      ...
      <Button type="button" size="sm" variant="ghost" disabled={operations.length === 1} onClick={() => setOperations(old => old.filter((_, i) => i !== index))}>Remove operation</Button>
    </div>)}<Button type="button" variant="ghost" size="sm" disabled={operations.length >= 30} onClick={() => setOperations(old => [...old, { ...initialOperation, id: `operation_${old.length + 1}` }])}>Add operation</Button>

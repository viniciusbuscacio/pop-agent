...
function ClientEditor({ client, onCancel, onSaved }: {
    ...
    <h4 className="font-medium">Allowed operations</h4>{operations.map((op, index) => <div key={index} className="space-y-2 border-t border-[var(--border)] pt-3">
      ...
      <Button type="button" size="md" variant="ghost" disabled={operations.length === 1} onClick={() => setOperations(old => old.filter((_, i) => i !== index))}>Remove operation</Button>
    </div>)}<Button type="button" variant="ghost" size="md" disabled={operations.length >= 30} onClick={() => setOperations(old => [...old, { ...initialOperation, id: `operation_${old.length + 1}` }])}>Add operation</Button>
    <div className="flex flex-wrap gap-2"><Button type="submit" disabled={busy}>Save</Button><Button type="button" variant="ghost" disabled={busy} onClick={onCancel}>Cancel</Button></div>
  </form></Card>;
}
function ClientCall({ client, enabled }: {
    ...
    return <div className="mt-3"><Button type="button" variant="ghost" size="md" disabled={!enabled || !client.enabled} onClick={() => setOpen(v => !v)}>Try operation</Button>{open ? <form onSubmit={e => void call(e)} className="mt-3 space-y-3"><Select id={`call-${client.id}`} label="Operation" value={operation} onChange={e => setOperation(e.target.value)}>{client.operations.map(op => <option key={op.id} value={op.id}>{op.method} {op.name}</option>)}</Select><TextArea id={`query-${client.id}`} label="Query parameters (JSON string values)" value={query} onChange={e => setQuery(e.target.value)}/><TextArea id={`body-${client.id}`} label="JSON body (optional, not for GET)" value={body} onChange={e => setBody(e.target.value)}/><p className="text-sm">This sends a real request. Write operations may change remote data.</p><div className="flex gap-2"><Button type="submit" disabled={busy}>Send request</Button><Button type="button" variant="ghost" disabled={busy} onClick={() => setOpen(false)}>Cancel</Button></div><pre className="max-h-72 overflow-auto whitespace-pre-wrap break-all text-sm">{result}</pre></form> : null}</div>;
}

...
export function RestAllowedIps() {
  ...
  useEffect(() => {
    ...
    void integrationsService.allowedIps().then(result => { if (active) setEntries(result.entries); }).catch(() => { if (active) setError('Could not load allowed IP addresses.'); });
    return () => { active = false; };
  }, [retry]);
  async function save(next: string[], adding = false): Promise<void> {
    if (busy) return;
    setBusy(true); setError('');
    try { setEntries((await integrationsService.setAllowedIps(next)).entries); if (adding) setDraft(''); }
  ...
  return <Card className="space-y-3">
    ...
    <p className="text-sm text-[var(--muted)]">Only these addresses can call the REST API. A valid access key is always required. By default, only 127.0.0.1/32 is allowed.</p>

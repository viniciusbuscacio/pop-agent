...
Retrieve the immutable v0.3.0 installer over HTTPS into an owner-only
...
Run `./deploy/local-release.sh publish` again from the same clean verified
batch. The publisher accepts an existing tag only for that commit, verifies

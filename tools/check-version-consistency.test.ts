...
describe('global version consistency', () => {
  ...
  it('reports package, lockfile, CLI and desktop release drift together', () => {
    ...
    writeFileSync(cliPath, readFileSync(cliPath, 'utf8').replace('0.2.25', '0.2.9'));
    ...
    expect(versionConsistencyErrors(target)).toEqual(expect.arrayContaining([
      'package.json version is "0.2.9"; expected 0.2.25',
      'package-lock.json packages["cli"] version is "0.2.9"; expected 0.2.25',
      'cli/src/version.ts VERSION is "0.2.9"; expected 0.2.25',
      'desktop/native/VERSION is "0.2.8"; expected 0.2.25',
      'desktop/pack/release.json version is "0.2.7"; expected 0.2.25',
      'desktop/pack/release.json file is "pop-desktop-0.2.7-darwin-arm64.zip"; expected pop-desktop-0.2.25-darwin-arm64.zip',
    ]));
  });
});

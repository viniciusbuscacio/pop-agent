...
describe('independent server release clients', () => {
  ...
  it.each(['tools/pack-cli.ts', 'tools/pack-node-runtime.ts', 'tools/cli-pack-files.ts',
    'tools/windows-setup.ts', 'tools/macos-setup.ts', 'tools/macos-tray-artifacts.ts',
    'tools/node-runtime-release.json', 'cli/tsconfig.json', 'shared/tsconfig.json', 'tsconfig.base.json'])('blocks changed client build input %s', (file) => {
    const f = fixture(); f.put(file, 'original input'); f.commit();
    const before = clientInputs(f.root);
    f.put(file, 'changed input'); f.commit();
    expect(clientInputs(f.root)).not.toBe(before);
  });

...
& $go build -trimpath -ldflags '-s -w -H windowsgui' -o (Join-Path $bin 'Pop Desktop Manager.exe') .
...
Copy-Item (Join-Path $bin 'Pop Desktop Manager.exe'),(Join-Path $bin 'Pop Desktop.exe'),(Join-Path $bin 'Pop Desktop.exe.version'),(Join-Path $bin 'Pop Desktop.ico') $payload -Force
& $go build -trimpath -ldflags '-s -w -H windowsgui' -o (Join-Path $bin 'Pop Desktop Setup.exe') .\cmd\pop-desktop-setup
if ($LASTEXITCODE -ne 0) { throw 'Setup build failed.' }
Remove-Item -Force (Join-Path $payload 'Pop Desktop Manager.exe'),(Join-Path $payload 'Pop Desktop.exe'),(Join-Path $payload 'Pop Desktop.exe.version'),(Join-Path $payload 'Pop Desktop.ico')
